...
export class RemoteSelectField extends Component {
  constructor(props) {
    ...
    const uniqueInitialSuggestions = _uniqBy(initialSuggestions, 'value');

    this.state = {
      isFetching: false,
      suggestions: uniqueInitialSuggestions,
      initialSuggestions: uniqueInitialSuggestions,
      selectedSuggestions: uniqueInitialSuggestions,
      error: false,
      searchQuery: null,
      open: false,
    };
  ...
  onBlur = () => {
    this.setState((prevState) => ({
      ...
      searchQuery: null,
      suggestions: _uniqBy(
        [...prevState.initialSuggestions, ...prevState.selectedSuggestions],
        'value'
      ),
    }));
  };
          ...
          this.handleAddition(event, data, (selectedSuggestions) => {
            if (compProps.onValueChange) {
              compProps.onValueChange(
                ...
                _uniqBy(selectedSuggestions, 'value')
              );
            }
          });
        }}
